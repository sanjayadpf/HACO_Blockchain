...
#please do not touch this. usage is implied throughout files
...
class ContractInterface(object):
	...
	default_vars_path = os.path.join(os.getcwd(), 'deployments/')
	if not os.path.exists(default_vars_path):
		os.makedirs(default_vars_path)
	...
	def __init__(
		...
		self.deployment_vars_path = deployment_vars_path + 'deployment' + str(random.getrandbits(20)) + '.json'
		self.web3.eth.default_account = web3.eth.accounts[0]
	...
	def deploy_contract(self, *constr_args, deployment_params=None):
		...
		for compiled_contract_key in self.all_compiled_contracts.keys():
			if self.contract_to_deploy in compiled_contract_key:
				...
				deployment_estimate = deployment.constructor(*constr_args).estimate_gas(transaction=deployment_params)
				#print(deployment_estimate)

				if deployment_estimate < self.max_deploy_gas:
					tx_hash = deployment.constructor(*constr_args).transact(transaction=deployment_params)
	...
	def get_instance(self):
		...
		contract_bytecode_length = len(self.web3.eth.get_code(self.contract_address).hex())
	...
	def send (self, function_, *tx_args, event=None, tx_params=None):
		...
		if True:
			...
			#print((f"Transaction receipt mined with hash: {receipt['transactionHash'].hex()} " f"on block number {receipt['blockNumber']} " f"with a total gas usage of {receipt['cumulativeGasUsed']}\n"))
